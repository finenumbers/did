...
class NumbersCatalogNormalized(Base, TimestampMixin):
    ...
    # UNVERIFIED: currency not in uploaded docs for either provider
    ...
    status_normalized: Mapped[str | None] = mapped_column(Text, nullable=True)
    has_sms: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    tariff_name: Mapped[str | None] = mapped_column(Text, nullable=True)
