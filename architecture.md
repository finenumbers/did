...
| Area | Path | Role |
|---|---|---|
| Providers | `backend/app/providers/` | Doc-driven adapters (SipOut, Runexis, UIS, Aurora, Exolve, Voximplant, MCN, Finenumbers, DIDWW) |
...
| DIDWW coverage | `backend/app/modules/didww/` + `didww_catalog` | Isolated international section: own job type, own advisory lock, own page |
...
- Primary UI/API path: `POST /api/v1/sync/start` (unified only). DIDWW is the one exception: `POST /api/v1/didww/sync` runs a separate job (`sync_job_type = didww`, lock `88221003`) and is not part of `PROVIDER_ORDER` or the finalize stages.
