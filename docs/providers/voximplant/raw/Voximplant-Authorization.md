...
Most Voximplant Management API calls require authentication and authorization, which link API calls to your account. Service accounts and API keys are the primary authentication methods. For method-level HTTP documentation, see the [Management API reference](/api-reference/management-api/reference).
...
You can create tokens [manually](#json-web-token).
...
To use most [Voximplant Management API](/api-reference/management-api/reference) functions, you need to generate a JSON Web Token. Learn how to do that via CLI and a bash script.
...
Read more about [authentication through service accounts](#json-web-token).

You can do the same via the [CreateKey](/api-reference/management-api/reference/role-system/create-key) method of the Management API.
...
You may need to run `chmod +x token.sh` to execute the script properly.
